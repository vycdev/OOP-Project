...
bool hasCharsInString(string str, const char* chr) {
	...
	for (int i = 0; i < strlen(chr); i++)
		if (str.find(chr[i]) != string::npos) ok = false;
	return ok;
}
...
void checkIfFloat(string str, int line) {
	...
	for (int i = 1; i < str.length() - 1; i++) {
		if (!isdigit(str[i])) { // the rest can be either a digit or a "."
			if (str[i] != '.') {
				throw err;
			}
		}
...
void lexer(string* tokens, int noOfTokens, int line) {
	...
	if (tokens[0] == "CREATE") {
		...
		checkNoOfTokensMin(noOfTokens, 2, line); checkToken(tokens[1], "TABLE", line);

		// check number of tokens then check if the next token which is the name of the table is valid.
		checkNoOfTokensMin(noOfTokens, 3, line); checkValidName(tokens[2], line); checkIfEmpty(tokens[2], "table name", line);
		...
		checkNoOfTokensMin(noOfTokens, 4, line); checkMaybeValidInput(tokens[3], line);
		...
		for (int i = 0; i < noOfColumns; i++) {
			...
			if (columnType == "integer") checkIfInt(columnDefaultValue, line);
			if (columnType == "float") checkIfFloat(columnDefaultValue, line);
